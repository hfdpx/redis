...
static int processTimeEvents(aeEventLoop *eventLoop) {
    ...
    // 如果系统的时钟被移动到未来，然后返回正确的值，那么时间事件可能会以一种随机的方式延迟，在这里，我们试图检测系统时间的倾斜，并在发生这种情况时强制所有的时间事件，早期处理事件比无限期延迟他们的危险要小，通过重置事件的运行时间,防止因时间穿插（skew）而造成的事件处理混乱
