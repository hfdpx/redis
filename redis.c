...
void initServer() {
    ...
    // SIGHUP:在用户终端连接正常或者非正常结束时发出，通常是在终端的控制进程结束时，通知同一session内的各个进程，这时他们不再与控制终端关联，redis作为守护进程，这时不会有其控制终端，所以首先忽略掉SIGHUP信号
    //SIGPIPE:该信号是在写管道发现读进程终止时产生的信号，写已终止的sockrt_stream同样会产生此信号，redis作为server，不可避免的会遇到各种各样的client，client意外终止导致产生的信号在server启动之后应该忽略.
